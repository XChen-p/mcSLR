function [T,x,w,flagw]=solveT2doublew(x,y,T,S,A,kGrid,kkGrid,rGrid,rkGrid,nT,w,flagw,gpu,meanT,ls)
...
Eprev=zeros(NT(1:6));%1*1*1*1*2
E=zeros(NT(1:6));%1*1*1*1*2

if gpu
    Eprev=gpuArray(Eprev);E=gpuArray(E);x=gpuArray(x);y=gpuArray(y);A=gpuArray(A);S=gpuArray(S);
end
...
%      end
    ...
    xTconj=conj(xT);
    Eprev(1,1,1,1,vS,:)=repmat(sum(sum(sum(sum(real(xT.*xTconj),1),2),3),4),[1 1 1 1 1 6]);
...
for t=1:6
        for k=1:NHe
            if a(1,k)==a(2,k)
                MHe(a(1,k),a(2,k))=dHe(k,s)+w(1,1,1,1,s,t);%*dHe(k,s);
            else
                MHe(a(1,k),a(2,k))=dHe(k,s);
                MHe(a(2,k),a(1,k))=dHe(k,s);
            end
        end
        tem=double(MHe)\double(dH(:,s));
        dH(t,s)=tem(t);
%        MHee(:,:,s)=MHe;
end
    end
...
Tupp=T;
Tupp(:,:,:,:,vS,t)=Tup(:,:,:,:,vS,t);
    et=precomputationsSincRigidTransform(kGrid,kkGrid,rkGrid,Tupp(:,:,:,:,vS,:),1,0);
    if gpu
        et{1}=gpuArray(et{1});
        for m=1:3
            for r=2:3
    ...
    xT=bsxfun(@times,xT,AA(:,:,:,:,vS));
    xTconj=conj(xT);
    E(1,1,1,1,vS,t)=sum(sum(sum(sum(real(xT.*xTconj),1),2),3),4);
end
    %     end
