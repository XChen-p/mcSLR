function [x,y,par]=solveX_SLR3Dii(x0,k_obj,Ttr,Tte,S,A,kGrid,rkGrid,nX,y,N,gpu,par,M)
% 
% Initialise x, z, y;
SH=conj(S);
...
if gpu
	...
	etInvtr{1}=gpuArray(etInvtr{1});
        for m=1:3
      	...
      	etDirte{1}=gpuArray(etDirte{1});
        for m=1:3
            for n=2:3
                etDirte{n}{m}=gpuArray(etDirte{n}{m});
            end
        end
	etInvte{1}=gpuArray(etInvte{1});
        for m=1:3
            for n=2:3
                etInvte{n}{m}=gpuArray(etInvte{n}{m});
            end
        end
end
