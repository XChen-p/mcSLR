function [T,x,w,winic,flagw]=solveT2test(x,y,T,S,A,kGrid,kkGrid,rGrid,rkGrid,nT,w,mA,flagw,gpu,meanT,winic)
...
multA=mA;%Factor to divide the weight (that regularizes the Hessian matrix) when H(end)<H(end-1)
...
dHe=single(zeros([NHe NT(5)]));%21*2
dH=single(zeros([NT(6) NT(5)]));%6*2
Eprev=single(zeros(NT(1:5)));%1*1*1*1*2
E=single(zeros(NT(1:5)));%1*1*1*1*2
...
%     q3(s)=-sum(real(sum(real(bsxfun(@times,MMHe(:,:,s),dH(:,s).')),2).*conj(dH(:,s))));
end
...
lambda=-1:0.1:1;
    for ii=1:length(lambda)
        ...
        TT=T+Tup*lambda(ii);
      et=precomputationsSincRigidTransform(kGrid,kkGrid,rkGrid,TT,1,0);
    xT=sincRigidTransform_XC(x,et,1,gpu);
    xT=forwardApplication(xT);
    xT=bsxfun(@minus,xT,y);
    xT=bsxfun(@times,xT,A(:,:,:,:,vS));
